    ...
    <div className="flex min-h-screen items-center justify-center bg-background p-4">
      <Card className="w-full max-w-md animate-fade-in border-2">
        <CardHeader className="text-center">
          <div className="mx-auto mb-4 flex h-14 w-14 items-center justify-center rounded-xl bg-primary shadow-[0_0_20px_-4px_hsl(var(--primary)/0.5)]">
            <TrendingUp className="h-7 w-7 text-primary-foreground" />
          </div>
          <CardTitle className="text-2xl font-black tracking-tighter">OpçõesX</CardTitle>
        ...
        <CardContent className="space-y-6">
          <Button 
            variant="outline" 
            className="w-full h-11 font-bold border-border/60 hover:bg-muted/50"
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <Button type="submit" className="w-full h-11 font-bold shadow-lg shadow-primary/20" disabled={submitting}>
              {submitting ? 'Processando...' : isSignUp ? 'Criar Conta Profissional' : 'Entrar no OpçõesX'}
            </Button>
          </form>
          <div className="text-center text-sm text-muted-foreground font-medium">
            {isSignUp ? 'Já possui uma conta?' : 'Ainda não tem acesso?'}{' '}
            <button onClick={() => setIsSignUp(!isSignUp)} className="text-primary font-bold underline-offset-4 hover:underline">
