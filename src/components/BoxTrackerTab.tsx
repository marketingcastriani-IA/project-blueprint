...
export default function BoxTracker() {
  ...
  const calculateBoxPairs = useCallback(
    (family: StockFamily): BoxPair[] => {
      ...
      strikeMap.forEach((group) => {
        ...
        // Ignora opções já vencidas (0 dias úteis) — não têm cotação nem fazem sentido para box
        if (vencimento && diasUteis !== null && diasUteis <= 0) return;
  ...
  useEffect(() => {
    ...
    if (cdiPercent >= notifThreshold) {
      ...
      // Push notification (background/mobile)
