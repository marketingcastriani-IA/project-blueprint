...
CREATE POLICY "Users can view legs of own analyses" ON public.legs FOR SELECT
  USING (EXISTS (SELECT 1 FROM public.analyses WHERE analyses.id = legs.analysis_id AND analyses.user_id = auth.uid()));
CREATE POLICY "Users can insert legs to own analyses" ON public.legs FOR INSERT
  WITH CHECK (EXISTS (SELECT 1 FROM public.analyses WHERE analyses.id = legs.analysis_id AND analyses.user_id = auth.uid()));
